...
//		Chip_SSP_DATA_SETUP_T data_setup;
	  ...
	  */
	 void PCD_WriteRegister( MFRC522Ptr_t mfrc,
			 	 	 	 	 uint8_t reg,		///< The register to write to. One of the PCD_Register enums.
	 						 uint8_t value		///< The value to write.
	 	 	 	 	 	 	) {
	 	// Set the settings to work with SPI bus
	 	Chip_SSP_SetFormat(mfrc->pSSP, SSP_BITS_8, SSP_FRAMEFORMAT_SPI, SSP_CLOCK_CPHA0_CPOL0);
	  ...
	  * The interface is described in the datasheet section 8.1.2.
	  */
	 void PCD_WriteNRegister( MFRC522Ptr_t mfrc,
			 	 	 	 	 uint8_t reg,		///< The register to write to. One of the PCD_Register enums.
	  ...
	  * The interface is described in the datasheet section 8.1.2.
	  */
	 uint8_t PCD_ReadRegister(	MFRC522Ptr_t mfrc,
	  ...
	  * The interface is described in the datasheet section 8.1.2.
	  */
	 void PCD_ReadNRegister(	 MFRC522Ptr_t mfrc,
			 	 	 	 	 uint8_t reg,		///< The register to read from. One of the PCD_Register enums.
...
//	 			values[index] = SPI.transfer(address);
	  ...
	  */
	 void PCD_SetRegisterBitMask(	 MFRC522Ptr_t mfrc,
	  ...
	  */
	 void PCD_ClearRegisterBitMask(  MFRC522Ptr_t mfrc,
			 	 	 	 	 	 	 uint8_t reg,	///< The register to update. One of the PCD_Register enums.
	  ...
	  */
	 StatusCode PCD_CalculateCRC(	 MFRC522Ptr_t mfrc,
	 	...
	 	PCD_SetRegisterBitMask(mfrc, FIFOLevelReg, 0x80);		// FlushBuffer = 1, FIFO initialization
	 	PCD_WriteNRegister(mfrc, FIFODataReg, length, data);	// Write data to the FIFO
	 	...
	 	uint8_t n;
	 	while (1) {
	 		n = PCD_ReadRegister(mfrc, DivIrqReg);	// DivIrqReg[7..0] bits are: Set2 reserved reserved MfinActIRq reserved CRCIRq reserved reserved
	 ...
	 void PCD_Init(MFRC522Ptr_t mfrc,LPC_SSP_T *pSSP) {
	 	...
	 	Chip_GPIO_SetPinDIROutput(LPC_GPIO_PORT, mfrc->_resetPowerDownPin.port, mfrc->_resetPowerDownPin .pin);

	 	if (Chip_GPIO_GetPinState(LPC_GPIO_PORT, mfrc->_resetPowerDownPin.port, mfrc->_resetPowerDownPin .pin) == false) {	//The MFRC522 chip is in power down mode.
	 		Chip_GPIO_SetPinState(LPC_GPIO_PORT, mfrc->_resetPowerDownPin.port, mfrc->_resetPowerDownPin .pin, (bool) true);		// Exit power down mode. This triggers a hard reset.
	 	...
	 	PCD_WriteRegister(mfrc,TPrescalerReg, 0xA9);		// TPreScaler = TModeReg[3..0]:TPrescalerReg, ie 0x0A9 = 169 => f_timer=40kHz, ie a timer period of 25�s.
	 	...
	 	PCD_AntennaOn(mfrc);						// Enable the antenna driver pins TX1 and TX2 (they were disabled by the reset)
	 } // End PCD_Init()
	  ...
	  * @return STATUS_OK on success, STATUS_??? otherwise.
	  */
	 StatusCode PCD_TransceiveData(	 MFRC522Ptr_t mfrc,
...
//		 checkCRC=false;
	 	...
	 	return PCD_CommunicateWithPICC(mfrc, PCD_Transceive, waitIRq, sendData, sendLen, backData, backLen, validBits, rxAlign, checkCRC);
	  ...
	  * @return STATUS_OK on success, STATUS_??? otherwise.
	  */
	 StatusCode PCD_CommunicateWithPICC( MFRC522Ptr_t mfrc,
...
//		checkCRC=false;
	 	...
	 	PCD_SetRegisterBitMask(mfrc, FIFOLevelReg, 0x80);			// FlushBuffer = 1, FIFO initialization
	 	PCD_WriteNRegister(mfrc, FIFODataReg, sendLen, sendData);	// Write sendData to the FIFO
	 	...
	 	i = 2000;
	 	while (1) {
	 		n = PCD_ReadRegister(mfrc, ComIrqReg);	// ComIrqReg[7..0] bits are: Set1 TxIRq RxIRq IdleIRq HiAlertIRq LoAlertIRq ErrIRq TimerIRq
	 	...
	 	if (backData && backLen) {
	 		n = PCD_ReadRegister(mfrc, FIFOLevelReg);			// Number of uint8_ts in the FIFO
	 		if (n > *backLen) {
	 			return STATUS_NO_ROOM;
	 		}
	 		*backLen = n;											// Number of uint8_ts returned
	 		PCD_ReadNRegister(mfrc, FIFODataReg, n, backData, rxAlign);	// Get received data from FIFO
	 		_validBits = PCD_ReadRegister(mfrc, ControlReg) & 0x07;		// RxLastBits[2:0] indicates the number of valid bits in the last received uint8_t. If this value is 000b, the whole uint8_t is valid.
	 		if (validBits) {
	  ...
	  * @return STATUS_OK on success, STATUS_??? otherwise.
	  */
	 StatusCode PICC_RequestA( MFRC522Ptr_t mfrc,
	  ...
	  * @return STATUS_OK on success, STATUS_??? otherwise.
	  */
	 StatusCode PICC_WakeupA( MFRC522Ptr_t mfrc,
			 	 	 	 	  uint8_t *bufferATQA,	///< The buffer to store the ATQA (Answer to request) in
	 						  uint8_t *bufferSize	///< Buffer size, at least two uint8_ts. Also number of uint8_ts returned if STATUS_OK.
	 										) {
	  ...
	  * @return STATUS_OK on success, STATUS_??? otherwise.
	  */
	 StatusCode PICC_REQA_or_WUPA(  MFRC522Ptr_t mfrc,
	 	...
	 	status = PCD_TransceiveData(mfrc, &command, 1, bufferATQA, bufferSize, &validBits, 0, false);
	 	if (status != STATUS_OK) {
	 		return status;
	 	}
...
//		validBits=0;
	 	...
	 	while (!uidComplete) {
	 		...
	 		while (!selectDone) {
	 			...
	 			if (currentLevelKnownBits >= 32) { // All UID bits in this Cascade Level are known. This is a SELECT.
	 				//DEBUGOUT(F("SELECT: currentLevelKnownBits=")); DEBUGOUT(currentLevelKnownBits, DEC);
	 				...
	 				//DEBUGOUT(F("ANTICOLLISION: currentLevelKnownBits=")); DEBUGOUT(currentLevelKnownBits, DEC);
	 			...
	 			result = PCD_TransceiveData(mfrc, buffer, bufferUsed, responseBuffer, &responseLength, &txLastBits, rxAlign, false);
	 ...
	 StatusCode PICC_HaltA(MFRC522Ptr_t mfrc) {
	 	...
	 	result = PCD_TransceiveData(mfrc, buffer, sizeof(buffer), NULL, 0, NULL, 0, false);
	  ...
	  */
	 StatusCode PCD_Authenticate( MFRC522Ptr_t mfrc,
	 	...
	 	return PCD_CommunicateWithPICC(mfrc, PCD_MFAuthent, waitIRq, &sendData[0], sizeof(sendData), NULL, 0, 0, 0, false);
	  ...
	  * @return STATUS_OK on success, STATUS_??? otherwise.
	  */
	 StatusCode MIFARE_Write( MFRC522Ptr_t mfrc,
	  ...
	  * @return STATUS_OK on success, STATUS_??? otherwise.
	  */
	 StatusCode MIFARE_Ultralight_Write( MFRC522Ptr_t mfrc,
	  ...
	  * @return STATUS_OK on success, STATUS_??? otherwise.
	  */
	 StatusCode MIFARE_Decrement( MFRC522Ptr_t mfrc,
	  ...
	  * @return STATUS_OK on success, STATUS_??? otherwise.
	  */
	 StatusCode MIFARE_Increment( MFRC522Ptr_t mfrc,
			 	 	 	 	 	  uint8_t blockAddr, ///< The block (0-0xff) number.
	  ...
	  * @return STATUS_OK on success, STATUS_??? otherwise.
	  */
	 StatusCode MIFARE_TwoStepHelper( MFRC522Ptr_t mfrc,
	  ...
	  * @return STATUS_OK on success, STATUS_??? otherwise.
	  */
	 StatusCode MIFARE_Transfer(MFRC522Ptr_t mfrc,	uint8_t blockAddr ///< The block (0-0xff) number.
	 											) {
	 	StatusCode result;
	 	uint8_t cmdBuffer[2]; // We only need room for 2 uint8_ts.
...
//	 		*value =l_24 | l_16 | l_8 | l_0;
	  ...
	  * @return STATUS_OK on success, STATUS_??? otherwise.
	  */
	 StatusCode PCD_NTAG216_AUTH(MFRC522Ptr_t mfrc, uint8_t* passWord, uint8_t pACK[]) //Authenticate with 32bit password
	 {
	 	StatusCode result;
	 	...
	 	uint8_t validBits		= 0;
	 	uint8_t rxlength		= 5;
	 	result = PCD_CommunicateWithPICC(mfrc, PCD_Transceive, waitIRq, cmdBuffer, 7, cmdBuffer, &rxlength, &validBits, 0, false);
	  ...
	  * @return STATUS_OK on success, STATUS_??? otherwise.
	  */
	 StatusCode PCD_MIFARE_Transceive(	MFRC522Ptr_t mfrc,
...
//		acceptTimeout = false;
	 	...
	 	uint8_t cmdBufferSize = sizeof(cmdBuffer);
	 	uint8_t validBits = 0;
	 	result = PCD_CommunicateWithPICC(mfrc, PCD_Transceive, waitIRq, cmdBuffer, sendLen, cmdBuffer, &cmdBufferSize, &validBits, 0, false);
	 ...
	 void PCD_DumpVersionToSerial(MFRC522Ptr_t mfrc) {
	 	...
	 	switch(v) {
	 		case 0x88: DEBUGOUT(" = (clone)\r\n");  break;
	 		case 0x90: DEBUGOUT(" = v0.0\r\n");     break;
	 		case 0x91: DEBUGOUT(" = v1.0\r\n");     break;
	 		case 0x92: DEBUGOUT(" = v2.0\r\n");     break;
	  ...
	  */
	 void PICC_DumpToSerial(MFRC522Ptr_t mfrc, Uid *uid	///< Pointer to Uid struct returned from a successful PICC_Select().
	 								) {
	 	MIFARE_Key key;
	  ...
	  * On success the PICC is halted after dumping the data.
	  */
	 void PICC_DumpMifareClassicToSerial( MFRC522Ptr_t mfrc,
